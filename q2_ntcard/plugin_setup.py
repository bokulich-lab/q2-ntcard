...
from q2_ntcard._methods import _estimate
from q2_ntcard._pipelines import estimate_kmer_histograms
from q2_ntcard._visualizers import _visualize

citations = Citations.load("citations.bib", package="q2_ntcard")
reads_type = SampleData[SequencesWithQuality | PairedEndSequencesWithQuality]
parameters = {
    "kmer_sizes": Str,
...
plugin = Plugin(
    ...
    description=(
        "Estimate k-mer cardinality and coverage histograms from "
...
plugin.methods.register_function(
    ...
    outputs=[("summary", ImmutableMetadata), ("histogram", ImmutableMetadata)],
    input_descriptions={
    ...
    description=(
        "Run ntCard over all reads and return analysis-ready "
...
plugin.visualizers.register_function(
    ...
    input_descriptions={
        "histogram": "Histogram table produced by ntCard.",
        "summary": "Summary table containing ntCard F0 estimates.",
    },
    parameter_descriptions={"title": "Title displayed above the chart."},
    name="Visualize ntCard k-mer histograms",
    description=(
        "Interactively inspect estimated k-mer multiplicity curves "
        "and their F0 cardinality estimates."
    ),
    citations=[citations["Mohamadi-Khan-Birol-2017"]],
)
